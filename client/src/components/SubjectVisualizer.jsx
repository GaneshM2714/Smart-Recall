...
const SubjectVisualizer = ({ subjectId, onClose }) => {
  ...
  // 👇 NEW: CONFIGURE PHYSICS FOR BETTER SPACING
  useEffect(() => {
    if (graphRef.current && !loading) {
      // 1. Charge: Repulsion (Negative number pushes nodes apart)
      // Default is usually around -30. We set it to -300 for more space.
      graphRef.current.d3Force('charge').strength(-300);

      // 2. Link: Length of lines
      // We set a minimum distance so cards don't bunch up on the topic
      graphRef.current.d3Force('link').distance(80);
      
      // 3. Re-heat simulation to apply changes
  ...
  const handleRate = async (rating) => {
    ...
    try {
      ...
      const { data: responseData } = await API.post('/study/review', {
        cardId: dbId,
        rating: rating,
        durationMs: 5000
      });
      ...
      const updatedNodes = responseData.updates;
      setData(prev => ({
        ...prev,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm animate-fade-in font-sans">
      ...
      >
        ...
        <ForceGraph2D
          ...
          d3VelocityDecay={0.2} // Increased slightly to stop jittering sooner
          d3AlphaDecay={0.02}
          linkColor={() => '#334155'}
      ...
      {selectedCard && (
        ...
        >
          <div className="w-[90%] md:w-full max-w-md max-h-[80vh] overflow-y-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-700 animate-in zoom-in duration-200 flex flex-col">
            ...
            {/* Body */}
            <div 
              className="flex-1 p-6 md:p-8 flex flex-col items-center justify-center text-center cursor-pointer relative min-h-[250px]" 
