...
test('Full User Journey: Signup -> Create Subject -> Add Card', async ({ page }) => {
  ...
  // Fill ONLY Email and Password (removed Username step)
  // FIXED: Using comma for OR logic, or just targeting specific types
  ...
  // FIXED: Be specific to avoid "strict mode" errors with multiple "Dashboard" texts
  await expect(page.getByRole('heading', { name: /dashboard/i })).toBeVisible();


  // --- 2. CREATE A SUBJECT ---
  // Click "New Sub" (or "New Subject")
  await page.click('text=New Sub'); 
  
  const subjectName = `E2E Subject ${Date.now()}`;
  
  // Fill the Subject Title (it's the first text input in the modal)
  await page.locator('input[type="text"]').first().fill(subjectName);
  
  // FIXED: Simplified selector. We find the "Save/Confirm" button and pick the last one (the one in the modal).
  // Added { force: true } to prevent backdrop overlay errors.
  ...
  // Save Topic (Again, use .last() to hit the modal button and force:true)
